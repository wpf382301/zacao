...
def inference_3conv(images, batch_size, n_classes, w1, b1, w2, b2, w3, b3, w4, b4, w5, b5):
...
def inference_3conv_without_dropout(images, batch_size, n_classes):
    with tf.name_scope('images'):
        tf.summary.image('images', images, batch_size)
    with tf.variable_scope('conv1'):
...
def inference(images, batch_size, n_classes, w1, b1, w2, b2, w3, b3, w4, b4, w5, b5, w6, b6):
    with tf.name_scope('images'):
        tf.summary.image('images', images, batch_size)
    with tf.variable_scope('conv1'):
        weights = tf.get_variable('weights', shape=w1.shape, initializer=tf.constant_initializer(w1), trainable=True)
    ...
    with tf.variable_scope('conv3'):
        ...
        conv3 = tf.nn.relu(pre_activation, name='conv3')
    with tf.variable_scope('pooling3_lrn'):
        pool3 = tf.nn.max_pool(conv3, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME', name='pooling3')
        norm3 = tf.nn.lrn(pool3, depth_radius=4, bias=1.0, alpha=0.001 / 9.0, beta=0.75, name='norm3')

    # 第四层
    with tf.variable_scope('conv4'):
        weights = tf.get_variable('weights', shape=w4.shape, dtype=tf.float32,
                                  initializer=tf.constant_initializer(w4))
        biases = tf.get_variable('biases', shape=b4.shape, dtype=tf.float32, initializer=tf.constant_initializer(b4))
        conv = tf.nn.conv2d(norm3, weights, strides=[1, 1, 1, 1], padding='SAME')
        pre_activation = tf.nn.bias_add(conv, biases)
        conv4 = tf.nn.relu(pre_activation, name='conv4')
    with tf.variable_scope('pooling4_lrn'):
        norm4 = tf.nn.lrn(conv4, depth_radius=4, bias=1.0, alpha=0.001 / 9.0, beta=0.75, name='norm4')
        pool4 = tf.nn.max_pool(norm4, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME', name='pooling4')
    ...
    with tf.variable_scope('local4'):
        weights = tf.get_variable('weights', shape=w6.shape, initializer=tf.constant_initializer(w6), trainable=True)
        biases = tf.get_variable('biases', shape=b6.shape, initializer=tf.constant_initializer(b6), trainable=True)
        local4 = tf.nn.relu(tf.matmul(local3, weights) + biases, name='local4')
